...
function renderHome() {
  ...
  const words = core.getLesson(catalog, active);
  const meta = core.lessonMeta(catalog, active);
  ...
  app.innerHTML = `
    <section class="hero"><div class="eyebrow" style="color:#fff">LESSON ${active + 1} / ${core.lessonCount(catalog)}</div><h1>${completed ? "本课已完成，继续巩固吧！" : "学5个词，赢一把盲盒钥匙"}</h1><p>${meta.title}：${words.map(word => word.word).join(" · ")}</p><button class="primary" data-action="start-home">${completed ? "复习本课" : learned ? "继续本课" : "开始本课"}</button><div class="mascot"></div></section>
    <section class="section grid-2"><div class="stat-card"><small>完整词库</small><strong>505</strong><small>共101课</small></div><div class="stat-card"><small>本课进度</small><strong>${learned}/5</strong><small>学习后才能开盒</small></div></section>
    <section class="section panel"><div style="display:flex;justify-content:space-between"><b>当前课程</b><span class="tag">约10分钟</span></div><div class="progress-track" style="margin:14px 0 9px"><div class="progress-fill" style="width:${learned * 20}%"></div></div><small class="muted">5个单词 · 5种检测 · 离线录音跟读</small></section>
    ...
    <section class="section"><h2>任务路线</h2><div class="mission"><span class="icon">📚</span><div><b>1. 学习5个新词</b><p>大图联想、离线发音、例句和记忆梗</p></div></div><div class="mission"><span class="icon">🎙️</span><div><b>2. 离线录音跟读</b><p>录音只留在本地，可立即回放</p></div></div><div class="mission"><span class="icon">🎁</span><div><b>3. 检测达标开盲盒</b><p>达到80分后获得学习钥匙</p></div></div></section>`;
}

function lessonRows() {
  return Array.from({ length: core.lessonCount(catalog) }, (_, index) => {
    const meta = core.lessonMeta(catalog, index);
...
function renderLessonsMarkup(rows) {
  return `<h2>全部101课</h2><div class="lesson-list">${rows.map(row => `<button class="lesson-card ${row.locked ? "locked" : ""} ${row.active ? "active" : ""}" data-action="open-lesson" data-index="${row.index}"><span class="lesson-icon">${row.locked ? "🔒" : row.completed ? "✅" : row.icon}</span><span class="lesson-main"><b>${row.title}</b><small>${row.subtitle}</small><span class="lesson-status">${row.completed ? "已完成，可复习" : row.locked ? "待解锁" : `${row.learned}/5 已学习`}</span></span><span class="result-arrow">›</span></button>`).join("")}</div>`;
}
...
function startLesson(index) {
  ...
  saveState();
  lessonIndex = index;
  lessonWords = core.getLesson(catalog, index);
...
function renderLearn() {
  if (!lessonWords.length) { lessonIndex = state.activeLesson; lessonWords = core.getLesson(catalog, lessonIndex); }
  if (learnPosition >= lessonWords.length) {
    app.innerHTML = `<div class="panel result-card"><div style="font-size:68px">🎯</div><h1>5个词已学习</h1><p class="muted">接下来完成五种检测。达到80分即可获得钥匙。</p><button class="primary wide" data-action="start-quiz">开始综合检测</button></div>`;
    return;
  }
...
function startQuiz() {
  quiz = core.buildQuiz(catalog, lessonIndex);
...
function renderQuizResult() {
  ...
  state = core.finishLesson(state, { score, voiceCompleted: quizAnswers.some(answer => answer.index === 4 && answer.correct), lessonIndex, lessonWordIds: lessonWords.map(word => word.id), totalLessons: core.lessonCount(catalog) });
  saveState();
